 ...
 */
void computeGaussParallelOpenMPCyclic(int nsize, int t) {
  int i, j, k;
  double pivotval;
  ...
  for (i = 0; i < nsize; i++) {
    ...
    #pragma omp parallel for \
      shared(t, i, nsize, pivotval, matrix, k, R) private(j) \
      schedule(static, CHUNKSIZE * t)
      for (j = i + 1; j < nsize; j++) {
        pivotval = matrix[j][i];
        matrix[j][i] = 0.0;
        for (k = i + 1; k < nsize; k++) {
 ...
 * for that particular pivot row.
 */
void computeGaussParallelOpenMPBlocked(int nsize) {
  int i, j, k;
  double pivotval;
  // looping over matrix columns
  for (i = 0; i < nsize; i++) {
    ...
    // Factorize the rest of the matrix:
    #pragma omp parallel for \
      shared(i, nsize, pivotval, matrix, k, R) private(j) \
      schedule(static, 3)
      for (j = i + 1; j < nsize; j++) {
        pivotval = matrix[j][i];
        matrix[j][i] = 0.0;
 ...
 * for that particular pivot row.
 */
void computeGaussParallelOpenMPDynamic(int nsize) {
  int i, j, k;
  double pivotval;
  // looping over matrix columns
  for (i = 0; i < nsize; i++) {
...
int main(int argc, char *argv[]) {
  ...
  int nsize = 0;
  double error;
  const double NUMSOLVES = 12.0;
  int t;

  if (argc != 4) {
    fprintf(stderr, "usage: %s <matrixfile> %s <t>\n", argv[0], argv[1]);
    exit(-1);
  }
   ...
   * If multiple "-t" flags are found, the last one will be used
   * If "-t" is the last command line argument, it will be ignored
   */
  ...
  // computeGaussParallelOpenMPCyclic()
  nsize = initMatrix(argv[1]);
  initRHS(nsize);
  initResult(nsize);
  gettimeofday(&start, 0);
  for (int iterator = 0; iterator < NUMSOLVES; ++iterator){
    computeGaussParallelOpenMPCyclic(nsize, t);
  }
  ...
  fprintf(stdout, "computeGaussParallelOpenMPCyclic\t%d\t%f\t%f\n", t, ((finish.tv_sec - start.tv_sec) + (finish.tv_usec - start.tv_usec) * 0.000001)/NUMSOLVES, error);
  
  // computeGaussParallelOpenMPBlocked()
  nsize = initMatrix(argv[1]);
  initRHS(nsize);
  initResult(nsize);
  gettimeofday(&start, 0);
  for (int iterator = 0; iterator < NUMSOLVES; ++iterator){
    computeGaussParallelOpenMPBlocked(nsize);
  }
  gettimeofday(&finish, 0);
  solveGauss(nsize);
  error = 0.0;
  ...
  fprintf(stdout, "computeGaussParallelOpenMPBlocked\t%d\t%f\t%f\n", t, ((finish.tv_sec - start.tv_sec) + (finish.tv_usec - start.tv_usec) * 0.000001)/NUMSOLVES, error);

  // computeGaussParallelOpenMPDynamic()
  nsize = initMatrix(argv[1]);
  initRHS(nsize);
  initResult(nsize);
  gettimeofday(&start, 0);
  for (int iterator = 0; iterator < NUMSOLVES; ++iterator){
    computeGaussParallelOpenMPDynamic(nsize);
  }
  gettimeofday(&finish, 0);
  solveGauss(nsize);
  error = 0.0;
  ...
  fprintf(stdout, "computeGaussParallelOpenMPDynamic\t%d\t%f\t%f\n", t, ((finish.tv_sec - start.tv_sec) + (finish.tv_usec - start.tv_usec) * 0.000001)/NUMSOLVES, error);
