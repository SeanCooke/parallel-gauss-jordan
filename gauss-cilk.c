 ...
 */
void computeGaussParallelCilk(int nsize) {
  int i, j, k;
  double pivotval;
...
int main(int argc, char *argv[]) {
  ...
  // computeGaussParallelCilk()
  snprintf(buffer, 12, "%d", t);
  ...
  for (int iterator = 0; iterator < NUMSOLVES; ++iterator){
    computeGaussParallelCilk(nsize);
  }
  ...
  fprintf(stdout, "computeGaussParallelCilk\t%d\t%f\t%f\n", __cilkrts_get_nworkers(), ((finish.tv_sec - start.tv_sec) + (finish.tv_usec - start.tv_usec) * 0.000001)/NUMSOLVES, error);
